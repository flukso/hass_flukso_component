...
async def async_setup_platform(hass, config, async_add_devices,
    ...
    def add_new_device(sensor):
        ...
        kubemotionsensor = KubeMotionDevice(hass=hass, name=name,
            state_topic="/sensor/" + sensor["id"] + "/" + sensor["data_type"],
            device_class=device_class, qos=0, force_update=False,
...
class KubeMotionDevice(BinarySensorDevice):
    ...
    def __init__(self, hass, name, state_topic, device_class,
                 qos, force_update, timeout, unique_id: Optional[str]):
        self._hass = hass
        self._name = name
        self._state = None
    ...
    async def async_added_to_hass(self):
        ...
        def state_message_received(topic, payload, qos):
            ...
            # start the timer
            _LOGGER.debug(
                "starting motion timeout for entity: %s with state_topic: %s",
                self._name, self._state_topic)

            if self._timeout > 0:
                def _delay_turn_off(now):
                    _LOGGER.debug("%s called delayed (%s sec) turn off", self.
                        _name, self._timeout)
                    self._state = False
                    self.async_schedule_update_ha_state()
                    self._timer = None

                self._timer = async_track_point_in_utc_time(self._hass,
                    _delay_turn_off, utcnow() + timedelta(seconds=self.
                    _timeout))
                self.async_schedule_update_ha_state()
            else:
                _LOGGER.error("Timeout for entity: %s has a bad value: %d",
                    self._name, self._timeout)

        await self._hass.data[FLUKSO_CLIENT].async_subscribe(self.
            _state_topic, state_message_received, self._qos, "utf-8")
